...
function Uploader(options) {
    ...
    service = {
        ...
        addFile: function(file, callback) {
            if (file) {
                let fileKey = CryptoJS.MD5(file.name + ':' + file.size).toString()
                let fileObj
        ...
        getDNS: function(bucketname, callback) {
            ...
            if (service.edgeNodeList) {
                ...
                if (opts.protocol === 'https:') {
                    let centerUrl = ['https://nosup-hz1.127.net']
        ...
        uploadTrunk: function(param, trunkData, callback) {
            ...
            xhr.onreadystatechange = function() {
                ...
                if (xhr.status === 200) {
                    ...
                    // 服务器出错重试
                    if (service.uTRetryCount < opts.retryCount + 1 && service.uTRetryCount > 0) {
                        // 同一个边缘节点重试两次
                        service.getOffset(
                            {
        ...
        upload: function(param, callback) {
            ...
            service.getDNS(param.bucketName, function(edgeNodeList) {
